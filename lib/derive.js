...
export async function derive (document, selectivePointers, options) {
  ...
  const revealDocument = selectJsonLd({ document: doc, pointers: combinedPointers })
  ...
  await canonicalize(deskolemizedNQuads.join(''),
    { documentLoader: options.documentLoader, inputFormat: 'application/n-quads', canonicalIdMap })
