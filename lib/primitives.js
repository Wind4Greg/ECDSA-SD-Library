 ...
 * @param {Object} options.prefix - a custom URN scheme (prefix)
...
export function skolemizeExpandedJsonLd (expanded, options) {
  ...
  expanded.forEach(element => {
    ...
    if (typeof element !== 'object' || element['@value'] !== undefined) {
      skolemizedExpanded.push(klona(element))
    } else {
    ...
    //   value as its only element for expanded and keeping the other parameters the same.
      ...
      // to the concatenation of "urn:", prefix, random, "_" and the value of count, incrementing
      ...
      if (skolemizedNode['@id'] === undefined) {
        ...
        // of the @id property in skolemizedNode to the concatenation of "urn:", prefix,
        // and the existing value of the @id property.
        skolemizedNode['@id'] = options.prefix + '_' + skolemizedNode['@id']
      }
      // Append skolemizedNode to skolemizedExpandedDocument.
      skolemizedExpanded.push(skolemizedNode)
    }
  })
  return skolemizedExpanded
}
